...
namespace solim {
	class Inference {
	public:
		static void inferMap(EnvDataset *eds, vector<Prototype>* prototypes, string targetVName, double threshold, string outSoilFile, string outUncerFile);
		static void iPSMInferSoil(vector<string> filenames, vector<string> datatypes, vector<string> layernames,
			...
			string outSoil, string outUncer, double ramEfficient);
		//static void typeInference();
		//static void propertyInference();
	};


	void Inference::inferMap(EnvDataset *eds, vector<Prototype>* prototypes, string targetVName, double threshold, string outSoilFile, string outUncerFile) {
		...
		double *envValues = new double[MAXLN];
		double *nodata = new double[MAXLN];
		...
		for (int i = 0; i < block_size; ++i) {
			...
			for (int n = 0; n < nx*ny; ++n) {
				...
				if (n % int(nx*ny*0.1) == 0) {
					progress++;
					cout << n <<" "<< nx*ny << " "<<progress<< endl;
				}
				//e = new EnvUnit();
				for (int k = 0; k < eds->Layers.size(); ++k) {
	...
	void Inference::iPSMInferSoil(vector<string> filenames, vector<string> datatypes, vector<string> layernames,
		double threshold, string sampleFilename, string targetVName, string idName,
		string outSoilFile, string outUncerFile, double ramEfficient) {
		...
		inferMap(eds, prototypes, targetVName, threshold, outSoilFile, outUncerFile);
	}
